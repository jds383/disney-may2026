    ...
    <div style={{ margin: "0", borderTop: "1px solid rgba(0,0,0,0.06)", background: "#FAFAF8" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 22px 6px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <span style={{ fontSize: 13, fontWeight: "bold", color: "#1A1A1A", fontFamily: "'Plus Jakarta Sans', sans-serif" }}>{info.flight}</span>
          <span style={{ fontSize: 11, color: "#888" }}>{info.from} → {info.to}</span>
          <span style={{ fontSize: 10, background: statusColor + "22", color: statusColor, border: `1px solid ${statusColor}44`, borderRadius: 20, padding: "1px 8px", fontFamily: "'Plus Jakarta Sans', sans-serif" }}>{d.status}</span>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          <div style={{ width: 5, height: 5, borderRadius: "50%", background: isLive ? "#27AE60" : "#CCC", boxShadow: isLive ? "0 0 4px #27AE60" : "none" }} />
          <span style={{ fontSize: 9, color: "#AAA", fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", padding: "0 22px 12px", gap: "4px 0" }}>
        {[
          ...
          <div key={label} style={{ padding: "4px 0" }}>
            <div style={{ fontSize: 9, color: "#AAA", fontFamily: "'Plus Jakarta Sans', sans-serif", letterSpacing: "0.08em", textTransform: "uppercase" }}>{label}</div>
...
function QuickServiceDining({ color }) {
  ...
  return (
    <div style={{ borderTop: "1px solid #F5F0EA", background: "#FAFAF8" }}>
      <div style={{ padding: "10px 22px 6px", fontSize: 12, color: "#888", fontFamily: "'Plus Jakarta Sans', sans-serif", letterSpacing: "0.08em", textTransform: "uppercase" }}>
        ...
        <div style={{ display: "flex", gap: 8, padding: "0 22px 10px" }}>
          {reservations.map((r, i) => (
            <div key={i} style={{ flex: 1, background: "#FAFAF8", border: "1px solid #EDE8E1", borderRadius: 8, padding: "8px 10px" }}>
              ...
              <div style={{ fontSize: 10, color: "#AAA", fontFamily: "'Plus Jakarta Sans', sans-serif", letterSpacing: "0.04em", marginBottom: 2 }}>#{r.conf}</div>
...
const days = [
  {
    ...
    highlights: [
      ...
      { icon: "🚤", text: "8:35 - 10:05 PM · Electrical Water Pageant", url: "https://disneyworld.disney.go.com/entertainment/magic-kingdom/electrical-water-pageant/" },
  ...
  },
  {
    ...
    parkId: "mk",
    highlights: [
      { icon: "🏰", text: "8:30 AM Early Entry · 9:00 AM Park Open · 10:00 PM Park Close", url: "https://disneyworld.disney.go.com/calendars/" },
  ...
  },
  {
    ...
    highlights: [
      ...
      { icon: "👸", text: "11:25 AM · Akershus Princess Storybook Dining", url: "https://disneyworld.disney.go.com/dining/epcot/akershus-royal-banquet-hall/menus/breakfast/", reservations: [
        { party: "S + M Family", time: "11:25 AM", size: "9 guests", conf: "356081980073" },
      ]},
      { icon: "🎆", text: "9:00 PM · Luminous: The Symphony of Us Fireworks", url: "https://disneyworld.disney.go.com/entertainment/epcot/luminous-the-symphony-us/" },
      { icon: "🌙", text: "9:00–11:00 PM · Extended Evening Hours" },
    ]
  },
  {
...
function Countdown() {
  ...
  if (mode === "done") return <div style={{ textAlign: "center", padding: "12px 0 20px" }}><div style={{ fontSize: 20, color: "#C8A96E", fontFamily: "Georgia, serif", fontStyle: "italic" }}>See ya real soon! 👋🏰</div></div>;
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 16, padding: "4px 0 20px" }}>
      <div style={{ position: "relative", width: 82, height: 88, flexShrink: 0, background: "#F0EDE8", borderRadius: 10, boxShadow: "0 6px 18px rgba(0,0,0,0.15), inset 0 1px 0 rgba(255,255,255,0.9)", border: "1px solid #D5CFC7", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" }}>
        ...
        <span style={{ fontSize: 72, fontWeight: "bold", color: "#1C2B4A", fontFamily: "'Plus Jakarta Sans', sans-serif", lineHeight: 1, userSelect: "none", letterSpacing: "-2px" }}>{String(value).padStart(2, "0")}</span>
      ...
      <div>
        <div style={{ fontSize: 16, letterSpacing: "0.12em", color: "#1C2B4A", fontFamily: "'Plus Jakarta Sans', sans-serif", fontWeight: "bold" }}>{sublabel}</div>
        {mode === "trip" && <div style={{ fontSize: 11, color: "#C8A96E", fontFamily: "Georgia, serif", fontStyle: "italic", marginTop: 4 }}>See ya real soon! 🎉</div>}
      </div>
    </div>
  );
}
...
function WeatherStack({ weather, error }) {
  if (error==="not yet available") return <div style={{textAlign:"right",flexShrink:0}}><div style={{fontSize:20,lineHeight:1,marginBottom:4}}>📅</div><div style={{fontSize:9,color:"rgba(255,255,255,0.35)",fontFamily:"'Plus Jakarta Sans',sans-serif",whiteSpace:"nowrap"}}>not yet available</div></div>;
  if (error) return <div style={{textAlign:"right",flexShrink:0}}><div style={{fontSize:18,lineHeight:1,marginBottom:4}}>⚠️</div><div style={{fontSize:9,color:"rgba(255,255,255,0.5)",fontFamily:"'Plus Jakarta Sans',sans-serif",maxWidth:100,wordBreak:"break-all"}}>{error}</div></div>;
  if (!weather) return <div style={{textAlign:"right",flexShrink:0}}><div style={{fontSize:16,lineHeight:1,marginBottom:4}}>🌡️</div><div style={{fontSize:9,color:"rgba(255,255,255,0.35)",fontFamily:"'Plus Jakarta Sans',sans-serif",whiteSpace:"nowrap"}}>fetching...</div></div>;
  return (
    <div style={{textAlign:"right",flexShrink:0}}>
      <div style={{fontSize:24,lineHeight:1,marginBottom:4}}>{weather.stormWindow?"⛈️":weather.icon}</div>
      <div style={{fontSize:11,color:"rgba(255,255,255,0.9)",fontFamily:"'Plus Jakarta Sans',sans-serif",lineHeight:1.5,whiteSpace:"nowrap"}}><span style={{color:"#FFF",fontWeight:"bold"}}>{weather.high}°</span><span style={{color:"rgba(255,255,255,0.5)",fontSize:9}}> ↑{weather.highTime}</span></div>
      <div style={{fontSize:11,color:"rgba(255,255,255,0.9)",fontFamily:"'Plus Jakarta Sans',sans-serif",lineHeight:1.5,whiteSpace:"nowrap"}}><span style={{color:"rgba(255,255,255,0.75)"}}>{weather.low}°</span><span style={{color:"rgba(255,255,255,0.5)",fontSize:9}}> ↓{weather.lowTime}</span></div>
    </div>
  );
}
...
function WeatherAlert({ weather }) {
  ...
  return (
    <div style={{ display:"flex",alignItems:"center",gap:8,padding:"7px 22px",background:"rgba(0,0,0,0.08)",borderBottom:"1px solid rgba(0,0,0,0.06)",fontSize:11,color:"rgba(255,255,255,0.9)",fontFamily:"'Plus Jakarta Sans',sans-serif" }}>
      <span>⛈️</span><span>{label} {start}–{end} · {prob}% chance</span>
    </div>
  );
}

export function Itinerary({ llPlanByPark }) {
  ...
  // LL Plan for this day's park
  const llPlan = day.parkId && llPlanByPark ? llPlanByPark[day.parkId] : null;
  const [llOpen, setLlOpen] = useState(false);

  return (
    <div style={{ minHeight: "100vh", background: "#FBF7F2", fontFamily: "Georgia, serif", padding: "28px 20px" }}>
      <div style={{ maxWidth: 480, margin: "0 auto" }}>
        {isToday("2026-05-21") && <Fireworks />}
        <div style={{ marginBottom: 24 }}>
          <h1 style={{ fontSize: 24, fontWeight: "normal", margin: "0 0 6px 0", letterSpacing: "-0.02em", color: "#1A1A1A" }}>Disney World May 2026</h1>
        </div>
        <Countdown />
        ...
        <div style={{ display:"flex",gap:6,marginBottom:20,overflowX:"auto",paddingBottom:4 }}>
          {days.map((d, i) => {
            ...
            return (
              <button key={i} onClick={() => setActiveDay(i)} style={{ flexShrink:0,padding:"6px 10px",borderRadius:20,border:"none",background:activeDay===i?days[i].color:"#EDE8E1",color:activeDay===i?"#FFF":"#888",fontSize:11,fontFamily:"'Plus Jakarta Sans',sans-serif",cursor:"pointer" }}>
                {num}{s}
              </button>
            );
          })}
        ...
        <div onPointerDown={onPointerDown} onPointerUp={onPointerUp} style={{ background:"#FFF",borderRadius:16,overflow:"hidden",boxShadow:"0 4px 24px rgba(0,0,0,0.08)",border:"1px solid #EDE8E1",touchAction:"pan-y",userSelect:"none" }}>
          ...
          <div style={{ background: day.color }}>
            <div style={{ padding:"20px 22px",display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:12 }}>
              <div style={{ flex:1,minWidth:0 }}>
                <div style={{ fontSize:10,letterSpacing:"0.15em",textTransform:"uppercase",color:"rgba(255,255,255,0.6)",fontFamily:"'Plus Jakarta Sans',sans-serif",marginBottom:4 }}>
                  ...
                  <div style={{ display:"flex",gap:8,marginTop:6 }}>
                    {day.rooms.map((r,ri) => (
                      <div key={ri} style={{ display:"flex",alignItems:"center",gap:4 }}>
                        <span style={{ fontSize:9,color:"rgba(255,255,255,0.45)",fontFamily:"'Plus Jakarta Sans',sans-serif",letterSpacing:"0.08em" }}>{r.label}</span>
                        <input value={rooms[`${activeDay}-${ri}`]||""} onChange={e => { const key=`${activeDay}-${ri}`; setRooms(prev => { const next={...prev,[key]:e.target.value}; try{localStorage.setItem("dw2026-rooms",JSON.stringify(next));}catch(_){} return next; }); }} placeholder="Room #" onClick={e=>e.stopPropagation()} style={{ fontSize:10,width:58,background:"rgba(255,255,255,0.15)",border:"1px solid rgba(255,255,255,0.25)",borderRadius:4,padding:"2px 5px",color:"#FFF",fontFamily:"'Plus Jakarta Sans',sans-serif",outline:"none" }} />
                      </div>
                    ))}
                  </div>
                )}
            ...
            {day.highlights.map((h, hi) => (
              <div key={hi}>
                {h.alternatives ? (
                  ...
                  <>
                    {!h.reservations && (
                      <div style={{ display:"flex",alignItems:"flex-start",gap:12,padding:"11px 22px",borderBottom:!h.flight&&!h.quickService&&hi<day.highlights.length-1?"1px solid #F5F0EA":"none" }}>
                        <span style={{ fontSize:16,flexShrink:0,marginTop:1 }}>{h.icon}</span>
                        {h.url ? <a href={h.url} target="_blank" rel="noopener noreferrer" style={{ fontSize:13,color:day.color,lineHeight:1.5,textDecoration:"underline",textDecorationStyle:"dotted",textUnderlineOffset:3 }}>{h.text} ↗</a> : <span style={{ fontSize:13,color:"#2A2A2A",lineHeight:1.5 }}>{h.text}</span>}
                      </div>
                    )}
                    {h.reservations && <div style={{ borderBottom:hi<day.highlights.length-1?"1px solid #F5F0EA":"none" }}><ReservationBadges reservations={h.reservations} color={day.color} icon={h.icon} text={h.text} url={h.url} /></div>}
                    {h.quickService && <QuickServiceDining color={day.color} />}
                    {h.flight && FLIGHTS[activeDay] && <div style={{ borderBottom:hi<day.highlights.length-1?"1px solid #F5F0EA":"none" }}><FlightStatus dayIndex={activeDay} color={day.color} /></div>}
                  </>
                )}
              </div>
            ))}
          ...
          {/* LL Plan section — only on park days */}
          {day.parkId && (
            <div style={{ borderTop:"1px solid #EDE8E1" }}>
              <div onClick={() => setLlOpen(o => !o)} style={{ display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 22px",cursor:"pointer",background:llOpen?"#F8F5F0":"#FFF" }}>
                <span style={{ fontSize:14,fontFamily:"Georgia, serif",color:"#1A1A1A" }}>⚡ Lightning Lane Plan</span>
                <span style={{ fontSize:11,color:"#AAA",transition:"transform 0.2s",display:"inline-block",transform:llOpen?"rotate(180deg)":"none" }}>▼</span>
              </div>
              {llOpen && llPlan && (
                <div style={{ padding:"4px 22px 16px" }}>
                  {llPlan.length === 0 && <div style={{ fontSize:12,color:"#AAA",fontFamily:"'Plus Jakarta Sans',sans-serif" }}>No selections made yet</div>}
                  {llPlan.map((item, i) => (
                    <div key={i} style={{ display:"flex",alignItems:"center",gap:8,padding:"7px 0",borderBottom:i<llPlan.length-1?"1px solid #F5F0EA":"none" }}>
                      <span style={{ fontSize:9,fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:700,padding:"2px 7px",borderRadius:8,background:item.badgeBg,color:item.badgeColor,border:`1px solid ${item.badgeBorder}`,whiteSpace:"nowrap",flexShrink:0 }}>{item.badge}</span>
                      <a href={item.url} target="_blank" rel="noreferrer" style={{ fontSize:13,color:"#1A1A1A",textDecoration:"underline",textDecorationStyle:"dotted",textUnderlineOffset:2 }}>{item.name} ↗</a>
                    </div>
                  ))}
                </div>
              )}
              {llOpen && !llPlan && <div style={{ padding:"8px 22px 16px",fontSize:12,color:"#AAA",fontFamily:"'Plus Jakarta Sans',sans-serif" }}>Loading...</div>}
            </div>
          )}
        </div>
        ...
        <div style={{ display:"flex",justifyContent:"space-between",marginTop:16 }}>
          <button onClick={() => goTo(activeDay-1)} disabled={activeDay===0} style={{ padding:"8px 20px",borderRadius:20,border:"1px solid #EDE8E1",background:"#FFF",color:activeDay===0?"#CCC":"#555",fontSize:13,cursor:activeDay===0?"default":"pointer",fontFamily:"Georgia, serif" }}>← Prev</button>
          <button onClick={() => goTo(activeDay+1)} disabled={activeDay===days.length-1} style={{ padding:"8px 20px",borderRadius:20,border:"1px solid #EDE8E1",background:"#FFF",color:activeDay===days.length-1?"#CCC":"#555",fontSize:13,cursor:activeDay===days.length-1?"default":"pointer",fontFamily:"Georgia, serif" }}>Next →</button>
        </div>

        <div style={{ marginTop:32,fontSize:10,color:"#CCC",textAlign:"center",fontFamily:"'Plus Jakarta Sans',sans-serif",letterSpacing:"0.1em" }}>DISNEY WORLD — MAY 2026</div>
      </div>
    </div>
  );
}
