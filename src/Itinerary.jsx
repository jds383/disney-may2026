...
const fmtHour = (t) => { try { return new Date(t).toLocaleTimeString(“en-US”,{hour:“numeric”,minute:“2-digit”}); } catch(*){return “”;} };
const WEATHER_CACHE_KEY = “dw2026-weather-cache”;
const getCachedWeather = (dk) => { try { const raw=localStorage.getItem(WEATHER_CACHE_KEY); if(!raw)return null; const cache=JSON.parse(raw); const entry=cache[dk]; if(!entry)return null; const ttl=(dk===new Date().toISOString().split(“T”)[0]||dk===new Date(Date.now()+86400000).toISOString().split(“T”)[0])?3600000:86400000; if(Date.now()-entry.fetchedAt<ttl)return entry.weather; return null; } catch(*){return null;} };
const setCachedWeather = (dk,w) => { try { let cache={}; try{const raw=localStorage.getItem(WEATHER_CACHE_KEY);if(raw)cache=JSON.parse(raw);}catch(*){} cache[dk]={weather:w,fetchedAt:Date.now()}; localStorage.setItem(WEATHER_CACHE_KEY,JSON.stringify(cache)); } catch(*){} };
